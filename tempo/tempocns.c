...
int main(int argc, char *argv[]) {
    ...
    //Calculo do tempo para deadline absoluto
    timespecAdd(ts, tsadd, &tsresult);
    
    clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME, &tsresult, NULL);   
    clock_gettime(CLOCK_MONOTONIC, &tsactual);
    
    printf("Valor decimal em ns. 1 ns = 1/1000000000 segundo\n");
    printf("Usando clock_nanosleep (exemplo de Deadline Absoluto)\n");
    printf("Inicio (tempo): %ld.%09ld\n", (long int)ts.tv_sec, (long int)ts.tv_nsec);    
    printf("Deadline prev.: %ld.%09ld\n", (long int)tsresult.tv_sec, (long int)tsresult.tv_nsec);
